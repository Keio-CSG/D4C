...
from d4c.solver.utils import DataSaverHook, StopForwardException
from d4c.quantization.quantized_module import QuantizedModule
from d4c.quantization.fake_quant import LSQFakeQuantize, LSQPlusFakeQuantize, QuantizeBase, AdaRoundFakeQuantize


def save_inp_oup_data(model, module, cali_text, cali_data: list, store_inp=False, store_oup=False, branch: str = 'visual', bs: int = 32, it: int = 1, keep_gpu: bool = True):
    ...
    with torch.no_grad():
        for i in range(int(it)):
                try:
                    if branch == 'visual':
                        _ = model(cali_data[i * bs: (i + 1) * bs].to(device), cali_text[0:1])
                    elif branch == 'text':
                        _ = model(cali_data[0:1].to(device), cali_text[i * bs: (i + 1) * bs])
                    else:
                        raise NotImplementedError("Branch must be visual or text.")
    ...
    if store_inp:
        cached[0] = torch.cat(cached[0], dim=0)
    if store_oup:
        cached[1] = torch.cat(cached[1], dim=0)
    handle.remove()
            ...
            for layer in self.module.modules():
                if isinstance(layer, (nn.Linear, nn.Conv2d)):
                    if hasattr(layer, 'weight_fake_quant'):
                        if isinstance(layer.weight_fake_quant, AdaRoundFakeQuantize):
...
def reconstruction(model, fp_model, module, fp_module, cali_text, cali_data, config):
    device = next(module.parameters()).device

    batch_size = {
        'visual': config.visual_batch_size,
        'text': config.text_batch_size
    }
    data = {
        'visual': cali_data,
        'text': cali_text
    ...
    scale_lr = {
        'visual': config.visual_scale_lr,
        'text': config.text_scale_lr
    }

    bs = batch_size[module.branch]
    it = data[module.branch].size(0) / bs

    # get data first
    quant_inp, _ = save_inp_oup_data(model, module, cali_text, cali_data, store_inp=True, store_oup=False, branch=module.branch, bs=bs, it=it, keep_gpu=config.keep_gpu)
    fp_inp, fp_oup = save_inp_oup_data(fp_model, fp_module, cali_text, cali_data, store_inp=True, store_oup=True, branch=module.branch, bs=bs, it=it, keep_gpu=config.keep_gpu)
    ...
    if len(a_para) != 0:
        a_opt = torch.optim.Adam(a_para, lr=scale_lr[module.branch])
    ...
    loss_func = LossFunction(module=module, weight=config.weight, iters=config.iters, b_range=config.b_range, warm_up=config.warm_up)

    sz = quant_inp.size(0)

    for i in range(config.iters):
        idx = torch.randint(0, sz, (batch_size[module.branch],))
